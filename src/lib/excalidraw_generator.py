...
class Diagram:
    ...
    def box(
        ...
        if shape_elem.get("boundElements") is None:
            shape_elem["boundElements"] = []
        shape_elem["boundElements"].append({"type": "text", "id": text_elem["id"]})
    ...
    def arrow_between(
        ...
        if use_orthogonal:
            ...
            # Store source/target for binding in _draw_elbowed_arrow
            self._current_arrow_source = source
            self._current_arrow_target = target
            self._draw_elbowed_arrow(sx, sy, points, color, label)
            self._current_arrow_source = None
            self._current_arrow_target = None
        else:
            ...
            # Add bindings to straight arrows too
            for e in elems:
                if e.get("type") == "arrow":
                    e["startBinding"] = {
                        "elementId": source.id, "focus": 0,
                        "gap": 1, "fixedPoint": None,
                    }
                    e["endBinding"] = {
                        "elementId": target.id, "focus": 0,
                        "gap": 1, "fixedPoint": None,
                    }
                    self._add_bound_element(source, e["id"])
                    self._add_bound_element(target, e["id"])
    ...
    def _draw_elbowed_arrow(
        ...
        """Draw an arrow with the given path points.

        When bound to source/target, uses simple 2-point arrows so
        Excalidraw can recalculate the route when shapes are dragged.
        """
        stroke = COLORS.get(color, "#1e1e1e")

        has_source = hasattr(self, '_current_arrow_source') and self._current_arrow_source is not None
        has_target = hasattr(self, '_current_arrow_target') and self._current_arrow_target is not None

        # When bound, use simple 2-point arrow for drag support
        if has_source and has_target:
            end_point = points[-1]
            simple_points = [[0, 0], end_point]
            ex, ey = sx + end_point[0], sy + end_point[1]

            elem = _base_element(
                "arrow", sx, sy,
                abs(end_point[0]) or 1, abs(end_point[1]) or 1,
                stroke_color=stroke,
                bg_color="transparent",
                roughness=self.style.roughness,
                stroke_style=self.style.stroke_style,
            ...
            elem["startBinding"] = {
                "elementId": self._current_arrow_source.id,
                "focus": 0, "gap": 1, "fixedPoint": None,
            }
            elem["endBinding"] = {
                "elementId": self._current_arrow_target.id,
                "focus": 0, "gap": 1, "fixedPoint": None,
            }
            self._add_bound_element(self._current_arrow_source, elem["id"])
            self._add_bound_element(self._current_arrow_target, elem["id"])
            elem.update({
            ...
            if label:
                label_x = sx + end_point[0] / 2
                label_y = sy + end_point[1] / 2 - 10
                label_elem = text(label_x, label_y, label,
                ...
                if elem.get("boundElements") is None:
                    elem["boundElements"] = []
                elem["boundElements"].append({"type": "text", "id": label_elem["id"]})
                self.elements.append(label_elem)
        else:
            # Unbound: use elbowed with manual waypoints
            ...
            if label and len(points) > 2:
                ...
                if abs(p2[0] - p1[0]) > abs(p2[1] - p1[1]):
                    label_y -= self.routing.label_offset_h
                else:
                    label_x += self.routing.label_offset_v
                label_elem = text(label_x, label_y, label,
                                  font_size=self.routing.label_font_size,
                                  font_family=self.box_style.font_family, color="black")
                self.elements.append(label_elem)
    ...
    def arrow_between_routed(
        ...
        # Draw the routed arrow with bound label and element bindings
        self._draw_routed_arrow_bound(waypoints, label=label, color=color,
                                       source=source, target=target)

    def _add_bound_element(self, shape_element: Element, arrow_id: str) -> None:
        """Register an arrow as a bound element on a shape."""
        # Find the shape's raw dict in self.elements
        idx = self._find_element_index(shape_element.id)
        if idx >= 0:
            elem = self.elements[idx]
            if elem.get("boundElements") is None:
                elem["boundElements"] = []
            elem["boundElements"].append({"type": "arrow", "id": arrow_id})
    ...
    def _draw_routed_arrow_bound(
        ...
        When source/target are provided, creates a simple 2-point bound arrow
        so Excalidraw can recalculate the route when shapes are dragged.
        """
        if len(waypoints) < 2:
            return
        ...
        # Build bindings to source/target elements
        start_binding = None
        end_binding = None
        if source is not None:
            ...
            self._add_bound_element(source, arrow_id)
        if target is not None:
            ...
            self._add_bound_element(target, arrow_id)

        # Use simple 2-point arrow when bound, so Excalidraw manages routing on drag
